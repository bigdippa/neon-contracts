...
contract NEONToken is Context, IERC20, Pausable {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "NEON::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "NEON::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "NEON::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "NEON::getPriorVotes: not yet determined");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying NEON tokens (not scaled);
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "NEON::_writeCheckpoint: block number exceeds 32 bits");
